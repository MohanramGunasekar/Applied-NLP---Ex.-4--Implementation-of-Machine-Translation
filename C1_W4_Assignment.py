...
def get_matrices(en_fr, french_vecs, english_vecs):
    ...
    # Sets of words that have embeddings
    english_set = set(english_vecs.keys())
    french_set = set(french_vecs.keys())

    # Loop through English-French pairs
    for en_word, fr_word in en_fr.items():
        # Ensure both words have embeddings
...
def compute_loss(X, Y, R):
    ...
    Outputs:
        loss: a scalar - the mean squared loss for given X, Y and R.
    '''
    ### START CODE HERE ###
    # number of rows in X
    m = X.shape[0]
...
def compute_gradient(X, Y, R):
    ...
    # compute the gradient: 2/m * X^T (XR - Y)
...
def align_embeddings(X, Y, train_steps=100, learning_rate=0.0003, verbose=True, compute_loss=compute_loss, compute_gradient=compute_gradient):
    ...
    Inputs:
        X: a matrix of dimension (m,n) where the rows are the English embeddings.
        Y: a matrix of dimension (m,n) where the rows correspond to the French embeddings.
        train_steps: positive int - number of gradient descent steps.
        learning_rate: positive float - step size for gradient descent.
    Outputs:
        R: a matrix of dimension (n,n) - the projection matrix that minimizes ||X R - Y||_F^2
    '''
    np.random.seed(129)

    # Initialize R as a square matrix with the same dimensionality as word vectors
    ...
    for i in range(train_steps):
        ...
        ### START CODE HERE ###
        # compute gradient using the function
        gradient = compute_gradient(X, Y, R)
...
def nearest_neighbor(v, candidates, k=1, cosine_similarity=cosine_similarity):
    """
    Input:
      - v: the vector you are finding the nearest neighbor for
    ...
    # compute cosine similarity between v and each candidate
...
def test_vocabulary(X, Y, R, nearest_neighbor=nearest_neighbor):
    '''
    Input:
        X: a matrix where the rows are the English embeddings.
        Y: a matrix where the rows correspond to the French embeddings.
        R: the transform matrix which translates English embeddings to French space.
    Output:
        accuracy: proportion of correct nearest neighbor predictions
    '''

    ### START CODE HERE ###
    # transform English embeddings
    pred = X @ R

    # initialize count of correct predictions
    num_correct = 0

    # loop over each transformed embedding
    for i in range(len(pred)):
        # find nearest neighbor index in Y
        pred_idx = nearest_neighbor(pred[i], Y, k=1)[0]

        # if nearest neighbor index equals the true index
        if pred_idx == i:
            num_correct += 1
...
def get_document_embedding(tweet, en_embeddings, process_tweet=process_tweet):
    ...
    # sum embeddings for each word in the tweet
...
def get_document_vecs(all_docs, en_embeddings, get_document_embedding=get_document_embedding):
    ...
    # dictionary mapping index to document embedding
    ind2Doc_dict = {}
...
def hash_value_of_vector(v, planes):
    ...
    Input:
        - v:  vector of tweet, shape (1, N_DIMS)
        - planes: matrix of shape (N_DIMS, N_PLANES) - hyperplanes dividing the space
    Output:
        - hash_value: integer hash for the vector
    """
    ### START CODE HERE ###
    # dot product between vector and all planes
    dot_product = v @ planes  # shape (1, N_PLANES)
    
    # get the sign of the dot product
    sign_of_dot_product = np.sign(dot_product)
    
    # convert sign to binary: 1 if >=0, 0 if <0
    ...
    # compute hash as sum of 2^i * h_i
    ...
    for universe_id in range(num_universes_to_use):
        ...
        for i, new_id in enumerate(new_ids_to_consider):
            ...
            if new_id not in ids_to_consider_set:
                ...
                # append it to the list of vectors to consider as possible nearest neighbors
